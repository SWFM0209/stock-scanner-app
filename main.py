...
<title>台股掃描器</title>
<style>
body { font-family: "Microsoft JhengHei"; background:#0f172a; color:white; padding:20px; }
button { padding:10px 16px; background:#2563eb; color:white; border:none; border-radius:8px; cursor:pointer; }
.card { background:#1e293b; padding:12px; border-radius:10px; margin-bottom:10px; cursor:pointer; }
.right { float:right; }
</style>
</head>
<body>
...
const stockNames = {
  ...
  "6669":"緯穎","3034":"聯詠","3711":"日月光","2881":"富邦金"
};
...
async function scan() {
  ...
  try {
    ...
    document.getElementById("status").innerText = "掃描失敗：Load failed";
  }
}
...
function renderList() {
  ...
  data.forEach(s => {
    const name = s.name || stockNames[s.symbol] || "";
    const div = document.createElement("div");
    div.className = "card";
    div.innerHTML = `
      <b>${name ? name + " " : ""}(${s.symbol})</b>
...
async function analyze(symbol) {
  ...
  try {
    ...
    const json = await res.json();
    document.getElementById("detail").innerText = json.llm_analysis || "無分析";
  } catch(e) {
